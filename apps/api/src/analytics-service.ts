...
import { buildAIRecommendationEvidence } from "./ai-recommendation-evidence";
...
export async function getInsights(context: AnalyticsContext) {
  ...
  const evidencePackets = buildAIRecommendationEvidence(unifiedRecommendations);
  return { insights: mappedInsights.map(({ updated: _updated, ...insight }) => insight), alerts, alertCount: alerts.length, evidence: behaviour.behaviour, aiEvidence: { ready: evidencePackets.filter((packet) => packet.readyForAI).length, packets: evidencePackets }, unavailableSources, message: uniqueCandidates.length ? undefined : "More traffic is needed before reliable CRO insights can be generated." };
}
