...
export function VisitorsView({ visitors }: Readonly<{ visitors: Visitor[] }>) {
    ...
    <div className="visitor-explorer">
      ...
      <div className="visitor-layout">
        <section className="visitor-list-panel" aria-label="Visitor list">
          <div className="list-heading"><div><p className="eyebrow">Sessions</p><h2>{filteredVisitors.length} visitors found</h2></div><span className="list-meta">Last 30 days</span></div>
...
function VisitorDetail({ visitor }: Readonly<{ visitor: Visitor }>) {
  return (
    <section className="visitor-detail" aria-label="Selected visitor details">
      ...
      <div className="detail-section"><div className="detail-section-heading"><h3>Visitor signals</h3><span className="signal-score">Engaged</span></div><div className="signal-grid"><div><span>Device</span><strong>{visitor.device}</strong></div><div><span>Country</span><strong>{visitor.country}</strong></div><div><span>Browser</span><strong>{visitor.browser}</strong></div><div><span>Source</span><strong>{visitor.source}</strong></div></div><div className="depth-row"><span>Scroll depth</span><strong>{visitor.scrollDepth}%</strong><div className="depth-bar"><i style={{ width: `${visitor.scrollDepth}%` }} /></div></div></div>
      <div className="detail-section"><div className="detail-section-heading"><h3>Session timeline</h3><span className="list-meta">{visitor.duration} duration</span></div><div className="timeline">{visitor.timeline.map((event) => <div className="timeline-event" key={`${event.time}-${event.title}`}><span className="timeline-icon">{event.icon}</span><div><strong>{event.title}</strong><p>{event.detail}</p></div><time>{event.time}</time></div>)}</div></div>
