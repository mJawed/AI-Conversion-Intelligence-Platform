...
function queryFor(days: number, organizationId: string, websiteId: string, offset = 0) { const to = new Date(); const from = new Date(to.getTime() - days * 86400000); return { organizationId, websiteId, from: from.toISOString(), to: to.toISOString(), limit: "50", offset: String(offset), sort: "recent" }; }
...
export default function VisitorsPage() {
  ...
  useEffect(() => {
    ...
    getAuthenticatedAnalytics<VisitorsResponse>("visitors", account.tokens.accessToken, queryFor(rangeDays, account.selectedOrganization.id, account.selectedWebsite.id)).then((result) => { if (!cancelled) { setLiveVisitors(result.visitors.map(mapVisitor)); setHasMore(result.pagination.hasMore); } }).catch((requestError) => { if (!cancelled) setError(requestError instanceof Error ? requestError.message : "Could not load visitors."); }).finally(() => { if (!cancelled) setIsLoading(false); });
    return () => { cancelled = true; };
  }, [account.selectedOrganization, account.selectedWebsite, account.tokens, rangeDays, retryKey]);

  async function loadMore() {
    if (useMockData || !hasMore || isLoadingMore || !account.tokens?.accessToken || !account.selectedOrganization || !account.selectedWebsite) return;
    setIsLoadingMore(true);
    try {
      const result = await getAuthenticatedAnalytics<VisitorsResponse>("visitors", account.tokens.accessToken, queryFor(rangeDays, account.selectedOrganization.id, account.selectedWebsite.id, liveVisitors.length));
      setLiveVisitors((current) => { const existing = new Set(current.map((visitor) => visitor.id)); return [...current, ...result.visitors.filter((visitor) => !existing.has(visitor.visitor_id)).map(mapVisitor)]; });
      setHasMore(result.pagination.hasMore);
    } catch (requestError) { setError(requestError instanceof Error ? requestError.message : "Could not load more visitors."); } finally { setIsLoadingMore(false); }
  }

  const visitors = useMockData ? mockVisitors : liveVisitors;
  return <DashboardShell activeHref="/visitors"><PageHeader eyebrow="Visitor analytics" title="Understand every journey." action={<Button>Export report ↓</Button>} /><div className="page-intro"><p>Explore sessions, identify intent, and find the moments that shape conversion.</p><select className="date-pill" value={rangeDays} onChange={(event) => setRangeDays(Number(event.target.value))} aria-label="Visitor date range">{ranges.map((range) => <option key={range.days} value={range.days}>{range.label}</option>)}</select></div>{!useMockData && account.isLoading ? <LoadingState /> : !useMockData && !account.selectedWebsite ? <EmptyState title="Select or connect a website" description="Create a website and install tracking before visitor journeys can appear." /> : !useMockData && error && !visitors.length ? <ErrorState message={error} onRetry={() => setRetryKey((key) => key + 1)} /> : !useMockData && isLoading ? <LoadingState /> : !useMockData && visitors.length === 0 ? <EmptyState title="No visitors in this period" description={`No visitor events were recorded in ${selectedRange.label.toLowerCase()}.`} /> : <VisitorsView visitors={visitors} live={!useMockData} hasMore={hasMore} loadingMore={isLoadingMore} onLoadMore={() => void loadMore()} error={error} />}</DashboardShell>;
}
