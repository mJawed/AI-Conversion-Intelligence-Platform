...
type BehaviourResponse = { behaviour: BehaviourRow[] };
...
function mapTargets(rows: BehaviourRow[]): ClickTarget[] { return rows.filter((row) => row.event_type === "click").map((row) => ({ selector: row.event_type, label: "Click events", clicks: String(row.events), rate: `${row.visitors} visitors` })); }
