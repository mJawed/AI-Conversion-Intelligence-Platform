...
import { archiveFunnel, createFunnel, duplicateFunnel, getAuthenticatedAnalytics, updateFunnel, type FunnelInput } from "../lib/api-client";
...
function queryFor(days: number, segment: string, organizationId: string, websiteId: string) { const to = new Date(); const from = new Date(to.getTime() - days * 86400000); return { organizationId, websiteId, from: from.toISOString(), to: to.toISOString(), limit: "50", offset: "0", segment }; }
...
export default function FunnelsPage() {
  ...
  useEffect(() => {
    ...
    getAuthenticatedAnalytics<FunnelsResponse>("funnels", account.tokens.accessToken, queryFor(rangeDays, segment, account.selectedOrganization.id, account.selectedWebsite.id)).then((result) => { if (!cancelled) { setLiveFunnels(result.funnels); setMessage(result.message); } }).catch((requestError) => { if (!cancelled) setError(requestError instanceof Error ? requestError.message : "Could not load funnels."); }).finally(() => { if (!cancelled) setIsLoading(false); });
    return () => { cancelled = true; };
  }, [account.selectedOrganization, account.selectedWebsite, account.tokens, rangeDays, segment, retryKey]);
  ...
  async function duplicateLive(funnelId: string) { if (!account.tokens?.accessToken || !account.selectedOrganization || !account.selectedWebsite) return; await duplicateFunnel(account.tokens.accessToken, account.selectedOrganization.id, account.selectedWebsite.id, funnelId); setRetryKey((key) => key + 1); }
  return <DashboardShell activeHref="/funnels"><PageHeader eyebrow="Funnel analytics" title="Find the conversion leaks." action={<Button>＋ Create funnel</Button>} /><div className="page-intro"><p>See where journeys break down and focus your next optimization on the highest-impact step.</p><select className="date-pill" value={rangeDays} onChange={(event) => setRangeDays(Number(event.target.value))} aria-label="Funnel date range">{ranges.map((range) => <option key={range.days} value={range.days}>{range.label}</option>)}</select><select className="date-pill" value={segment} onChange={(event) => setSegment(event.target.value)} aria-label="Funnel visitor segment"><option value="all">All visitors</option><option value="mobile">Mobile</option><option value="desktop">Desktop</option><option value="tablet">Tablet</option></select></div>{!useMockData && account.isLoading ? <LoadingState /> : !useMockData && !account.selectedWebsite ? <EmptyState title="Select or connect a website" description="Create a website and install tracking before funnels can appear." /> : !useMockData && error ? <ErrorState message={error} onRetry={() => setRetryKey((key) => key + 1)} /> : !useMockData && isLoading ? <LoadingState /> : <FunnelsView funnels={useMockData ? mockFunnels : liveFunnels} live={!useMockData} onSaveLive={saveLive} onArchiveLive={archiveLive} onDuplicateLive={duplicateLive} />}</DashboardShell>;
}
