...
import { getAdminAccess, getAdminBilling, getAdminCustomer, getAdminCustomers, getAdminOverview, getAdminUsage, type AdminBilling, type AdminCustomer, type AdminCustomerDetail, type AdminOverview, type AdminUsage } from "../lib/api-client";
...
export default function AdminPage() {
  ...
  return <DashboardShell activeHref="/admin"><PageHeader eyebrow="Platform administration" title="Run the customer base." action={<Button onClick={() => accessToken && void loadAdminData(accessToken)}>Refresh data</Button>} />
    ...
    {selectedCustomer && <section className="admin-panel admin-detail-panel"><div className="admin-panel-heading"><div><p className="eyebrow">Customer detail</p><h2>{selectedCustomer.name}</h2><span>{selectedCustomer.owner.email}</span></div><button className="button" onClick={() => setSelectedCustomer(null)} type="button">Close</button></div><div className="admin-detail-grid"><div><strong>Plan</strong><span>{selectedCustomer.plan} · {selectedCustomer.status}</span></div><div><strong>Members</strong><span>{selectedCustomer.members.length}</span></div><div><strong>Websites</strong><span>{selectedCustomer.websites.length}</span></div><div><strong>Events</strong><span>{formatNumber(selectedCustomer.usage.events)}</span></div></div><h3>Websites</h3><div className="admin-website-list">{selectedCustomer.websites.map((website) => <div key={website.id}><strong>{website.name}</strong><span>{website.domain} · {website.installationStatus}</span></div>)}</div></section>}
  </DashboardShell>;
}
