...
export function FormsView({ forms, live = false }: Readonly<{ forms: FormAnalytics[]; live?: boolean }>) {
...
function FieldDropoff({ form }: Readonly<{ form: FormAnalytics }>) {
  return <section className="widget field-widget" aria-labelledby="field-dropoff-heading"><div className="widget-heading"><div><p className="eyebrow">Form friction</p><h2 id="field-dropoff-heading">Field drop-off</h2></div><span className="list-meta">{form.name}</span></div>{form.fields.length ? <div className="field-table"><div className="field-table-heading"><span>Field</span><span>Completion</span><span>Drop-off</span><span>Signal</span></div>{form.fields.map((field) => <div className="field-row" key={field.name}><strong>{field.name}</strong><div className="field-progress"><i style={{ width: `${field.completion}%` }} /><span>{field.completion}%</span></div><b className={field.dropOff > 20 ? "dropoff-high" : "dropoff-low"}>{field.dropOff}%</b><span className="field-issue">{field.issue}</span></div>)}</div> : <p className="widget-empty">Field-level tracking is not available for this form yet.</p>}</section>;
}

function FormRecommendation({ recommendation }: Readonly<{ recommendation: FormAnalytics["recommendation"] | null }>) {
  if (!recommendation) return <section className="form-recommendation" aria-labelledby="form-recommendation-heading"><p className="eyebrow">CRO recommendation</p><h2 id="form-recommendation-heading">Recommendations need field-level evidence.</h2><p className="recommendation-reason">The current live endpoint provides starts and completions. Field-level tracking is needed before recommending a form change.</p></section>;
...
function CompletionChart({ available }: Readonly<{ available: boolean }>) {
  if (!available) return <section className="widget completion-widget" aria-labelledby="completion-heading"><div className="widget-heading"><div><p className="eyebrow">Time analysis</p><h2 id="completion-heading">Completion time</h2></div></div><p className="widget-empty">Completion-time events are not available yet.</p></section>;
