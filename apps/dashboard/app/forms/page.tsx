...
type LiveForm = { form_id: string; started: number; completed: number; completionRate: number };
...
function mapForm(form: LiveForm): FormAnalytics { const started = Number(form.started); const completed = Number(form.completed); return { id: form.form_id, name: form.form_id, path: "Not available", started: started.toLocaleString(), completed: completed.toLocaleString(), completionRate: `${Number(form.completionRate).toFixed(2)}%`, abandonmentRate: `${started ? (((started - completed) / started) * 100).toFixed(2) : "0.00"}%`, avgTime: "Not available", submissions: completed.toLocaleString(), fields: [], recommendation: { title: "Field-level analysis is not available yet", reason: "The live forms endpoint currently reports form starts and completions. Add field event capture to identify the highest-friction field.", impact: "Not available", priority: "Low" } }; }
