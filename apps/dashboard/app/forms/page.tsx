...
type LiveForm = { form_id: string; path: string; started: number; completed: number; errors: number; completionRate: number; abandonmentRate: number; avgCompletionTime?: string; fields?: Array<{ name: string; completion: number; dropOff: number; errors: number; issue: string }> };
...
function mapForm(form: LiveForm): FormAnalytics { const started = Number(form.started); const completed = Number(form.completed); const errors = Number(form.errors); const lowCompletion = started >= 5 && Number(form.completionRate) < 50; const fields = form.fields ?? []; const largestField = [...fields].sort((a, b) => b.dropOff - a.dropOff)[0]; return { id: form.form_id, name: form.form_id, path: form.path || "Not available", started: started.toLocaleString(), completed: completed.toLocaleString(), completionRate: `${Number(form.completionRate).toFixed(2)}%`, abandonmentRate: `${Number(form.abandonmentRate).toFixed(2)}%`, avgTime: form.avgCompletionTime || "Not available", submissions: completed.toLocaleString(), validationErrors: errors, fields, recommendation: { title: errors > 0 ? "Validation errors are creating form friction" : largestField && largestField.dropOff > 30 ? `${largestField.name} is the largest form drop-off` : lowCompletion ? "More than half of form starters abandon before submission" : "Form performance is being monitored", reason: errors > 0 ? `${errors} privacy-safe validation errors were recorded without storing field names or values.` : largestField && largestField.dropOff > 30 ? `${largestField.dropOff.toFixed(1)}% of starters did not reach the ${largestField.name} field interaction.` : lowCompletion ? `${Number(form.abandonmentRate).toFixed(1)}% of form starters did not submit the form.` : "Collect more form activity to identify a reliable optimization opportunity.", impact: errors > 0 || lowCompletion || Boolean(largestField && largestField.dropOff > 30) ? "+5–12% completions" : "Not available", priority: errors > 0 && lowCompletion ? "High" : errors > 0 || lowCompletion || Boolean(largestField && largestField.dropOff > 30) ? "Medium" : "Low" } }; }
